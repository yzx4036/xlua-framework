...
namespace AssetBundles
{
    ...
    public class AssetBundleManager : MonoSingleton<AssetBundleManager>
    {
        ...
        Dictionary<string, Action<string>> onBundleLoadComplateCB = new Dictionary<string, Action<string>>();
        ...
        protected int DecreaseReferenceCount(string assetbundleName)
        {
            ...
            {
                Debug.LogWarning("》》》》DecreaseReferenceCount.assetbundleName :" + count);
            }
            if (count <= 0)
            {
        ...
        public BaseAssetBundleAsyncLoader LoadAssetBundleAsync(string assetbundleName, out BaseAssetBundleAsyncLoader outLoader)
        {
#if UNITY_EDITOR
            if (AssetBundleConfig.IsEditorMode)
            {
                outLoader = new EditorAssetBundleAsyncLoader(assetbundleName);
                return outLoader;
            }
#endif
        ...
        /// <returns></returns>
        public void LoadFairyGUIBundle(string pFUIPackageName, FairyGUI.UILoadCallback pCallBack)
        {
            string path = "Assets/" + AssetBundleConfig.AssetsFolderName + "/UI/" + AssetBundleConfig.FairyGUIFolderName + "/" + pFUIPackageName;
            Debug.Log(">>>>>>>>>LoadFairyGUIBundle");
#if UNITY_EDITOR
            ...
            //else
            //{
            //RunLoadFairyGUIBundle(pFUIPackageName, pCallBack)
            //加载AssetBundle
            Debug.LogWarning(">>>>>>>>>>>>>>LoadFairyGUIBundle");
            StartCoroutine(RunLoadFairyGUIBundle(pFUIPackageName, pCallBack));

            Debug.LogWarning("LoadFairyGUIBundle finish !!!!!!!!!!!!!!");
            //this.LoadAssetBundleAsync
            //FairyGUI.UIPackage.AddPackage()
            //}
        }
        IEnumerator RunLoadFairyGUIBundle(string pFUIPackageName, FairyGUI.UILoadCallback pCallBack)
        {
            ...
            //bool descStatus = MapAssetPath(descBundlePath, out descBundleName, out descBundleAssetName);
            //bool resStatus = MapAssetPath(resBundlePath, out resBundleName, out resBundleAssetName);

            //Logger.LogError(">>>>>>>>>>>> :" + descBundleName);
            //if (!descStatus || !resStatus)
            //{
            //        Logger.LogError("No assetbundle at asset path :" + descBundlePath);
            //        yield return null;
            //}
            Debug.LogError(">>>>> descBundlePath" + descBundlePath + ">>> resLoader:" + resBundlePath);
            BaseAssetBundleAsyncLoader resLoader = null;
            this.LoadAssetBundleAsync(resBundlePath, out resLoader);
            yield return resLoader;
            var bundle = GetAssetBundleCache(resBundlePath);
            Debug.LogError(">>>>>  resLoader finish " + bundle + " addr:" + getMemory(resLoader.assetbundle));
            BaseAssetBundleAsyncLoader descLoader = null;
            this.LoadAssetBundleAsync(descBundlePath, out descLoader);
            yield return descLoader;
            Debug.LogError(">>>>> descLoader finish " + descLoader.assetbundle);

            UIPackage pkg = UIPackage.AddPackage(descLoader.assetbundle, resLoader.assetbundle);


            if (pCallBack != null)
            {
                Debug.Log(">>>>>>>>uisource callback ");
                pCallBack();
            }
            descLoader.Dispose();
            resLoader.Dispose();
        ...
        protected bool UnloadAssetBundle(string assetbundleName, bool unloadResident = false, bool unloadAllLoadedObjects = false, bool unloadDependencies = true)
        {
            ...
            if (assetbundleName.StartsWith("ui/fair") || assetbundleName.StartsWith("lua"))
            {
                Debug.Log("》》》loader.assetbundleName count" + count);
            }
            if (count > 0)
            {
                // 存在引用，还是被需要的，不能卸载
        ...
        public BaseAssetAsyncLoader LoadAssetAsync(string assetPath, System.Type assetType)
        {
            ...
            {
                BaseAssetBundleAsyncLoader assetbundleLoader = null;
                LoadAssetBundleAsync(assetbundleName, out assetbundleLoader);
        ...
        void OnProsessingAssetBundleAsyncLoader()
        {
            for (int i = prosessingAssetBundleAsyncLoader.Count - 1; i >= 0; i--)
            {
                ...
                {
                    // 解除加载器对AB持有的引用
                    //这里的写法 刚加载完成就释放掉 会导致bug
                    //int count = DecreaseReferenceCount(loader.assetbundleName);
                    //if (count <= 0)
                    //{
                    //    if (loader.assetbundleName.StartsWith("ui/fair"))
                    //    {
                    //        Debug.LogWarning("loader.assetbundleName " + loader.assetbundleName);
                    //    }
                    ...
                    if (count <= 0)
                    {
                        ...
                        UnloadAssetBundle(loader.assetbundleName);
                        if (loader.assetbundleName.StartsWith("ui/fair") || loader.assetbundleName.StartsWith("lua"))
                        {
                            Debug.LogWarning("loader.assetbundleName " + loader.assetbundle);
                        }
                    }
        ...
        void OnProsessingFUIAssetBundleAsyncLoader()
        {
