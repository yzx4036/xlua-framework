...
namespace AssetBundles
{
    ...
    public class AssetBundleAsyncLoader : BaseAssetBundleAsyncLoader
    {
        ...
        public static AssetBundleAsyncLoader Get()
        {
            if (pool.Count > 0)
            {
                var loader = pool.Dequeue();
                loader.isDispose = false;
                return loader;
            }
            else
            {
        ...
        public override void Update()
        {
            ...
            for (int i = waitingList.Count - 1; i >= 0; i--)
            {
                if (AssetBundleManager.Instance.IsAssetBundleLoaded(waitingList[i]))
                {
                    ...
                    if (curFinished == assetbundleName)
                    {
                        ...
                        {
                            Debug.LogError(">>>>>>>>>assetbundle " + assetbundle + AssetBundleManager.getMemory(assetbundle));
                        }
                    }
            ...
            if (isOver)
            {
                ...
                if (assetbundleName.StartsWith("ui/fair"))
                {
                    Debug.LogWarning(">>>>>>>>>over assetbundle " + assetbundle + " addr:" + AssetBundleManager.getMemory(this));
                }
            }
        }
        ...
        public override void Dispose()
        {
            if (assetbundleName.StartsWith("ui/fair"))
            {
                Debug.LogWarning("><<<<<<<<<<WWWWWWWWWWWWW Dispose");
            }
            waitingList.Clear();
            waitingCount = 0;
            Recycle(this);
